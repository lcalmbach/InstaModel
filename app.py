...
def main():
    ...
    st.subheader("Enter up to 10 keywords for your image search")
    ...
    if len(st.session_state.keywords) > 0:
        ...
        if len(st.session_state.image_dict) > 0:
            image_preview(st.session_state.image_dict)
            with st.spinner("Creating zip file..."):
                zip_buffer = create_zip(st.session_state.image_dict, "downloaded_images")
            st.success("✅ Zip file ready!")

            # Provide download button
            st.download_button(
                label="Download Zipped Images",
                data=zip_buffer,
                file_name="vivaldi_images.zip",
